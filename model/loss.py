...
class GenDynamicTargets(GenTargets):
    ...
    def target_assign_single_img(
        ...
        assign_result = self.assigner.assign(
            cls_preds.sigmoid(), center_priors, decoded_bboxes, gt_bboxes, gt_labels-1, self.strides)
...
def giou_loss(preds,targets):
    ...
    area1=(preds[:,2]+preds[:,0])*(preds[:,3]+preds[:,1])
    area2=(targets[:,2]+targets[:,0])*(targets[:,3]+targets[:,1])
    area_union = (area1+area2-overlap)
    iou=overlap/area_union
    lt_max = torch.max(preds[:,:2],targets[:,:2])
    rb_max = torch.max(preds[:,2:],targets[:,2:])
    wh_max = (rb_max+lt_max).clamp(min=0)
    cover = wh_max[:,0]*wh_max[:,1]
    other = ((cover - overlap)/cover).clamp(min=0)
    giou = iou - other
    loss = -((1 + giou)/2).clamp(min = 1e-6).log()
    return loss.sum()
...
class LOSS(nn.Module):
    ...
    def forward(self,inputs):
        ...
        reg_loss=compute_reg_loss(reg_preds,reg_targets,mask_pos,mode = 'giou').mean()
