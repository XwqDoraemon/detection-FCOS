...
def eval_ap_2d(gt_boxes, gt_labels, pred_boxes, pred_labels, pred_scores, iou_thread, num_cls):
    ...
    """
    all_ap = {}
    for label in range(num_cls+1)[1:]:
...
if __name__=="__main__":
    ...
    eval_dataset = dataset = BDD100kDataset(root_dir= "/hy-tmp/bdd100k",imgset='10k',resize_size=[640,960],is_train = False,scal_mutil=False,augment =None,mean= config.mean,std=config.std)
    ...
    # checkpoint = torch.load("/hy-tmp/training_dir_giou/model_24.pth")
    model.load_state_dict(torch.load("/hy-tmp/training_dir_giou/model_9.pth", map_location=torch.device('cpu'))["model_state_dict"])
